...
def count_files(dir_eq_count):
    ...
    for path, exp_count in dir_eq_count.items():
        ...
        if got_count != exp_count:
            res = False
            print("Expected {nexp} files in '{path}', 'count_files' got {ngot}".format(nexp=exp_count, path=path, ngot=got_count), file=sys.stderr)
            print("   found files: {}".format(found_files), file=sys.stderr)
...
def duplicates_dir(request, monkeypatch):
    ...
    if not has_files:
        raise Exception(duplicates_dir.__doc__)
...
def _check_test_file_name(out_dir, fn):
    assert not os.path.isabs(fn), "No absolute paths allowed, found '{}'".format(fn)
    assert not '..' in fn, "No .. allowed, found '{}'".format(fn)
    fn = os.path.join(out_dir, fn)
    if fn in _generated_test_file_names[out_dir]:
        raise Exception("File '{}' is generated by more than 1 decorator, or more than once by a decorator.".format(fn))
