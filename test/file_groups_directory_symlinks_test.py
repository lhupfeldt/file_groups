...
def test_file_groups_specified_symlinks_to_protect_dirs(duplicates_dir, capsys):
    """Specify a directory for a protect dir"""
    with FGC(FileGroups(["kisym"], ["df"], debug=True), duplicates_dir) as ck:
...
def test_file_groups_specified_symlinks_to_work_on_dirs(duplicates_dir, capsys):
    """Specify a directory for a protect dir"""
    with FGC(FileGroups(["ki"], ["dfsym"], debug=True), duplicates_dir) as ck:
...
def test_file_groups_found_symlinks_to_protect_dirs(duplicates_dir, capsys):
...
def test_file_groups_found_symlinks_to_work_on_dirs(duplicates_dir, capsys):
