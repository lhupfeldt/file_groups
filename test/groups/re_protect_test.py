...
@same_content_files('B', 'df/df/KEEP_ME.jpg')
def test_file_groups_group_dirs_protect_file_in_nested_may_work_on_dirs(duplicates_dir, log_debug):
    with FGC(FileGroups([], ['df'], config_handler=ConfigHandler(protect=[re.compile(r'KEEP_ME.jpg$')])), duplicates_dir) as ck:
        assert ck.ckfl('must_protect.files', 'df/df/KEEP_ME.jpg')


@same_content_files('B', 'ki/df/KEEP_ME.jpg')
def test_file_groups_group_dirs_protect_file_in_may_work_on_dir_under_protect_dir(duplicates_dir, log_debug):
    with FGC(FileGroups(['ki'], ['ki/df'], config_handler=ConfigHandler(protect=[re.compile(r'KEEP_ME.jpg$')])), duplicates_dir) as ck:
        assert ck.ckfl('must_protect.files', 'ki/df/KEEP_ME.jpg')
...
def test_file_groups_group_files_by_config_protect(duplicates_dir, set_conf_dirs, log_debug):
